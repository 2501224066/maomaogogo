...
func (c *CommentController) Post() {
	...
	models.AddNotice(article.User.UserID, "<a class='gray' href='/user_read/"+strconv.Itoa(user.UserID)+"'>"+user.Nickname+"</a>"+"在你发布的文章<a class='gray' href='/article_read/"+strconv.Itoa(article.ArticleID)+"'>"+article.Title+"</a>下发布了新评论")

	// 向@的人发送通知
	aUserIDArr := AnalysisCommentContent(input.Content)
	fmt.Println(aUserIDArr)
	for _, v := range aUserIDArr {
		aUserID, _ := strconv.Atoi(v)
		models.AddNotice(aUserID, "<a class='gray' href='/user_read/"+strconv.Itoa(user.UserID)+"'>"+user.Nickname+"</a>"+"在文章<a class='gray' href='/article_read/"+strconv.Itoa(article.ArticleID)+"'>"+article.Title+"</a>下提到了你")
	}
